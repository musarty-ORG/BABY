...
export default function DashboardPage() {
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "tokens" | "payments" | "subscription" | "activity">(
    "overview",
  )
  ...
  const currentPlan = PLAN_DETAILS[dashboardData.tokens.plan as keyof typeof PLAN_DETAILS] || PLAN_DETAILS.none
  ...
  return (
    <div className="min-h-screen bg-black text-white font-mono">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              <div className="text-right">
                <p className="text-sm text-gray-300">{dashboardData.user.email}</p>
                <p className="text-xs text-purple-400 capitalize">{dashboardData.user.role}</p>
              </div>
              <Link
                href="/"
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-4 py-8">
        {/* Welcome Section */}
        <div className="mb-8">
          <h2 className="text-3xl font-bold mb-2">Welcome back, {dashboardData.user.name || "Developer"}! 👋</h2>
          <p className="text-gray-400">Manage your Code Homie account, tokens, and subscriptions.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-gray-900/50 border border-purple-500/30 rounded-xl p-6">
            ...
            <p className="text-3xl font-bold text-white">{dashboardData.tokens.total_balance}</p>
          ...
          <div className="bg-gray-900/50 border border-blue-500/30 rounded-xl p-6">
            ...
            <p className="text-3xl font-bold text-white">{dashboardData.usage_stats.total_api_calls}</p>
          ...
          <div className="bg-gray-900/50 border border-cyan-500/30 rounded-xl p-6">
            ...
            <p className="text-3xl font-bold text-white">{dashboardData.usage_stats.avg_response_time}ms</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-green-400 capitalize">{dashboardData.user.status}</span>
                  </div>
                </div>
                <div>
                  <p className="text-gray-400 mb-2">Member Since</p>
                  <p className="text-white">{new Date(dashboardData.user.created_at).toLocaleDateString()}</p>
                </div>
                <div>
                  <p className="text-gray-400 mb-2">Last Login</p>
                  <p className="text-white">{new Date(dashboardData.user.last_login).toLocaleDateString()}</p>
                </div>
                <div>
                  <p className="text-gray-400 mb-2">Payment Methods</p>
                  <p className="text-white">{dashboardData.payment_methods.length} saved</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gray-900/50 border border-purple-500/30 rounded-xl p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="text-center">
                  ...
                  <p className="text-3xl font-bold text-purple-400">{dashboardData.tokens.monthly_balance}</p>
                </div>
                <div className="text-center">
                  <p className="text-gray-400 mb-2">Top-up Tokens</p>
                  <p className="text-3xl font-bold text-cyan-400">{dashboardData.tokens.topup_balance}</p>
                </div>
              </div>
            ...
            <div className="bg-gray-900/50 border border-purple-500/30 rounded-xl p-6">
              ...
              <div className="space-y-3">
                {dashboardData.token_history.slice(0, 10).map((entry) => (
                  <div key={entry.id} className="flex items-center justify-between py-3 border-b border-gray-700">
                    <div>
                    ...
                    <div className={`text-lg font-bold ${entry.delta > 0 ? "text-green-400" : "text-red-400"}`}>
                      {entry.delta > 0 ? "+" : ""}
                      {entry.delta}
                    </div>
                  </div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-gray-400 text-sm">
                            Expires {method.expiry} • Added {new Date(method.created_at).toLocaleDateString()}
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                    ...
                    <div className="text-right">
                      ...
                      <p
                        className={`font-semibold ${
                          dashboardData.subscription.status === "ACTIVE" ? "text-green-400" : "text-yellow-400"
                        }`}
                      >
            ...
            <div className="bg-gray-900/50 border border-purple-500/30 rounded-xl p-6">
              ...
              <div className="space-y-3">
                {dashboardData.recent_activity.map((activity) => (
                  <div key={activity.id} className="flex items-center justify-between py-3 border-b border-gray-700">
                    <div>
                      <p className="text-white font-medium">{activity.endpoint}</p>
                      <p className="text-gray-400 text-sm">
                        {activity.method} • {new Date(activity.timestamp).toLocaleString()}
                      </p>
                    </div>
                    <div className="flex items-center gap-4">
                      <span className="text-gray-400 text-sm">{activity.duration}ms</span>
                      <span
