...
export const GET = withErrorHandler(async (req: NextRequest) => {
  try {
    ...
    const tokenHistory = await databaseService.getUserTokenLedgerHistory(userId, 20)

    // Get vaulted payment methods
    const paymentMethods = await databaseService.getVaultedPaymentMethodsByUserId(userId)
    ...
    const recentActivity = await databaseService.getAnalyticsEventsByType("api_call", 10)
    const userActivity = recentActivity.filter((event) => event.user_id === userId)
        ...
        usage_stats: {
          ...
          avg_response_time:
            userActivity.length > 0
              ? Math.round(userActivity.reduce((sum, a) => sum + (a.duration || 0), 0) / userActivity.length)
              : 0,
        },
      },
    })
