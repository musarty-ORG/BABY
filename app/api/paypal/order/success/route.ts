...
export const GET = withErrorHandler(async (req: NextRequest) => {
  try {
    ...
    if (!orderId) {
      return NextResponse.redirect(`${process.env.NEXT_PUBLIC_BASE_URL}/pricing?error=missing_order_id`)
    }
    ...
    if (capturedOrder.status !== "COMPLETED") {
      console.error(`Order capture failed: ${orderId}, status: ${capturedOrder.status}`)
      return NextResponse.redirect(`${process.env.NEXT_PUBLIC_BASE_URL}/pricing?error=payment_failed`)
    }

    // Extract user email and calculate messages from order
    const userEmail = capturedOrder.payer.email_address
    const amount = Number.parseFloat(capturedOrder.purchase_units[0].amount.value)
    ...
    if (!messages) {
      ...
      return NextResponse.redirect(`${process.env.NEXT_PUBLIC_BASE_URL}/pricing?error=invalid_amount`)
    }
    ...
    await tokenLedger.creditUser(user.id, messages, `Top-up purchase: ${messages} messages ($${amount})`, "topup")
