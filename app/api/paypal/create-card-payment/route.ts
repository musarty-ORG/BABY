...
export const POST = withErrorHandler(async (req: NextRequest) => {
  try {
    ...
    if (!amount || !cardDetails || !billingAddress || !userEmail || !userId) {
      return NextResponse.json({ success: false, error: "Missing required payment information" }, { status: 400 })
    }
    ...
    if (savePaymentMethod) {
      ...
      if (!customer) {
        ...
        const paypalCustomer = await paypalService.createCustomer(userEmail, userId)
    ...
    if (savePaymentMethod && payment.payment_source?.card?.attributes?.vault?.id) {
