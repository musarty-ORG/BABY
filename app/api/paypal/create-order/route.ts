  ...
  try {
    ...
    if (!messages || !userEmail || !userId) {
      return NextResponse.json(
        { success: false, error: "Missing required fields: messages, userEmail, userId" },
        { status: 400 },
      )
    }
    ...
    if (!amount) {
      return NextResponse.json(
        { success: false, error: `Invalid message count: ${messages}. Available: 1, 5, 10, 25` },
        { status: 400 },
      )
    }
    ...
    console.log(`Creating PayPal order for user ${userId}: ${messages} messages ($${amount})`)

    // Create PayPal order
    const order = await paypalService.createOrder(amount, description, userEmail)
