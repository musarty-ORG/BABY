...
import { Send, Code, Download, FileText, Globe, Zap, CheckCircle, Clock, AlertTriangle, Palette, Upload, ExternalLink, Brain, Settings, Shield, Eye, EyeOff } from 'lucide-react'
...
`
  ...
  const getPreviewHTML = () => {
    ...
    const mainFile = pipelineResult.codeFiles["index.html"] || 
                     pipelineResult.codeFiles["app/page.tsx"] ||
                     pipelineResult.codeFiles["pages/index.tsx"] ||
                     Object.values(pipelineResult.codeFiles)[0]

    if (!mainFile) return "<html><body><h1>No preview available</h1></body></html>"

    // If it's a React component, create a basic HTML wrapper
    if (mainFile.includes("export default") || mainFile.includes("function") || mainFile.includes("const")) {
      return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
    <div class="preview-content">
        ...
        <ul>
            ${Object.keys(pipelineResult.codeFiles).map(file => `<li><code>${file}</code></li>`).join('')}
        </ul>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-purple-300/70">AI-POWERED. REAL-TIME. PRODUCTION-READY.</p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gray-800/30 border border-cyan-500/20 rounded-lg p-4">
                <div className="flex items-center justify-between mb-2">
                  <label className="block text-sm font-semibold text-cyan-400">AI GENERATION MODE:</label>
                  <div className="flex items-center gap-2 text-xs">
                    <Brain className="w-4 h-4 text-cyan-400" />
                    <span className="text-cyan-400">{useDualModel ? "DUAL MODEL (5X FASTER)" : "SINGLE MODEL"}</span>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  <button
