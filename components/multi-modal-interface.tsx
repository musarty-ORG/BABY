...
export default function MultiModalInterface({
  ...
  const [activeMode, setActiveMode] = useState<"voice" | "image" | "sketch" | "video" | null>(null)
  ...
  return (
    <div className="bg-gray-900/50 border border-green-500/30 rounded-lg p-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h3 className="text-lg font-semibold text-green-400">Multi-Modal AI Interface</h3>
        {currentProject && <span className="text-xs text-green-500/70">Project: {currentProject}</span>}
      </div>
      ...
      {activeMode === "image" && !uploadedFile && (
        <div className="mb-6">
          <div className="relative bg-gray-800/50 border border-green-500/30 rounded-lg p-4">
            <video ref={videoRef} className="w-full max-w-md mx-auto rounded-lg" autoPlay muted playsInline />
          ...
          <div className="bg-gray-800/50 border border-green-500/30 rounded-lg p-4">
            <div className="flex items-center justify-between mb-4">
              <h4 className="text-sm font-semibold text-green-400">
                {uploadedFile.type.startsWith("image/") ? "Image Preview" : "Video Preview"}
              </h4>
              <button onClick={clearFile} className="text-red-400 hover:text-red-300 transition-colors">
            ...
            {uploadedFile.type.startsWith("image/") ? (
              ...
              <video src={previewUrl} controls className="w-full max-w-md mx-auto rounded-lg mb-4" />
            )}
      ...
      {isProcessing && (
        <div className="bg-yellow-900/20 border border-yellow-500/30 rounded-lg p-4 mb-6">
          ...
          <p className="text-xs text-yellow-300/70 mt-1">Converting your input to code. This may take a moment.</p>
        </div>
      )}
      ...
      {isListening && (
        <div className="bg-purple-900/20 border border-purple-500/30 rounded-lg p-4">
          <div className="flex items-center gap-2 text-purple-400">
            ...
            <span className="text-sm font-semibold">Listening for voice commands...</span>
          </div>
          <p className="text-xs text-purple-300/70 mt-1">
            Try saying: "Make the header bigger", "Add a search button", "Change the color to blue"
          </p>
        </div>
      )}
      ...
      <div className="border-t border-green-500/30 pt-4">
        <h4 className="text-sm font-semibold text-green-400 mb-3">Quick Examples:</h4>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
          <div className="bg-gray-800/30 rounded p-3">
            <div className="flex items-center gap-2 mb-2">
              <Mic className="w-3 h-3 text-purple-400" />
              <span className="font-semibold text-purple-400">Voice Commands</span>
            </div>
