...
export default function PredictiveInsightsPanel({
  ...
  const [selectedTab, setSelectedTab] = useState<"smells" | "refactoring" | "dependencies" | "insights">("insights")
    ...
    return (
      <div className="bg-gray-900/50 border border-green-500/30 rounded-lg p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="text-lg font-semibold text-green-400">AI Code Analysis</h3>
        </div>
  ...
  if (!analysis) {
    return (
      <div className="bg-gray-900/50 border border-green-500/30 rounded-lg p-6">
        ...
        <p className="text-green-500/70">Generate code to see predictive insights and suggestions.</p>
      </div>
    )
  }
    ...
    <div className="bg-gray-900/50 border border-green-500/30 rounded-lg">
      ...
      <div className="p-4 border-b border-green-500/30">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Brain className="w-6 h-6 text-green-400" />
            <h3 className="text-lg font-semibold text-green-400">Predictive Code Evolution</h3>
          </div>
          <button
            onClick={analyzeCode}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
          <div className="bg-red-900/20 border border-red-500/30 rounded p-3">
            <div className="text-red-400 text-sm font-semibold">Critical Issues</div>
            <div className="text-red-300 text-xl font-bold">
              {analysis.codeSmells.filter((s) => s.severity === "critical").length}
            </div>
          </div>
          <div className="bg-blue-900/20 border border-blue-500/30 rounded p-3">
            <div className="text-blue-400 text-sm font-semibold">Refactoring Ops</div>
            <div className="text-blue-300 text-xl font-bold">{analysis.refactoringOpportunities.length}</div>
          </div>
          <div className="bg-yellow-900/20 border border-yellow-500/30 rounded p-3">
            <div className="text-yellow-400 text-sm font-semibold">Dependency Alerts</div>
            <div className="text-yellow-300 text-xl font-bold">{analysis.dependencyAlerts.length}</div>
          </div>
          <div className="bg-purple-900/20 border border-purple-500/30 rounded p-3">
            <div className="text-purple-400 text-sm font-semibold">AI Insights</div>
            <div className="text-purple-300 text-xl font-bold">{analysis.predictiveInsights.length}</div>
          </div>
        </div>
      </div>
      ...
      <div className="flex border-b border-green-500/30">
        {[
          ...
          { id: "smells", label: "Code Smells", icon: <AlertTriangle className="w-4 h-4" /> },
          { id: "refactoring", label: "Refactoring", icon: <Wrench className="w-4 h-4" /> },
        ...
        {selectedTab === "smells" && (
          <div className="space-y-3">
            {analysis.codeSmells.length === 0 ? (
              <p className="text-green-500/70 text-center py-8">No code smells detected. Great job! 🎉</p>
            ) : (
              analysis.codeSmells.map((smell, index) => (
                <div key={index} className={`border rounded-lg p-3 ${getSeverityColor(smell.severity)}`}>
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        {getTypeIcon(smell.type)}
                        <span className="font-semibold text-sm">{smell.description}</span>
                        <span className={`px-2 py-1 rounded text-xs font-semibold ${getSeverityColor(smell.severity)}`}>
                          {smell.severity.toUpperCase()}
                        </span>
                      </div>
                      <p className="text-xs opacity-70 mb-2">📁 {smell.location.file}</p>
                      <p className="text-sm opacity-90 mb-2">{smell.suggestion}</p>
                      <p className="text-xs opacity-70">💪 {smell.estimatedImpact}</p>
                    </div>
        ...
        {selectedTab === "refactoring" && (
          <div className="space-y-3">
            {analysis.refactoringOpportunities.length === 0 ? (
              <p className="text-green-500/70 text-center py-8">No refactoring opportunities found.</p>
            ) : (
              analysis.refactoringOpportunities.map((opportunity, index) => (
                <div key={index} className="border border-blue-500/30 rounded-lg p-3 bg-blue-900/20">
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <Wrench className="w-4 h-4 text-blue-400" />
                        <span className="font-semibold text-sm text-blue-400">{opportunity.description}</span>
                      </div>
                      <p className="text-xs text-blue-300/70 mb-2">📁 {opportunity.file}</p>
                    </div>
                    <button
                  ...
                  {expandedItems.has(`refactor-${index}`) && (
                    <div className="space-y-3 mt-3 pt-3 border-t border-blue-500/20">
                      <div>
                        <h5 className="text-xs font-semibold text-blue-400 mb-1">Benefits:</h5>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <h5 className="text-xs font-semibold text-blue-400 mb-1">Before:</h5>
                          <pre className="text-xs bg-gray-800/50 p-2 rounded overflow-x-auto">
                            {opportunity.beforeCode}
                          </pre>
                        </div>
                        <div>
                          <h5 className="text-xs font-semibold text-blue-400 mb-1">After:</h5>
                          <pre className="text-xs bg-gray-800/50 p-2 rounded overflow-x-auto">
        ...
        {selectedTab === "dependencies" && (
          <div className="space-y-3">
            {analysis.dependencyAlerts.length === 0 ? (
              <p className="text-green-500/70 text-center py-8">All dependencies are up to date! 🚀</p>
            ) : (
              analysis.dependencyAlerts.map((alert, index) => (
                <div key={index} className="border border-yellow-500/30 rounded-lg p-3 bg-yellow-900/20">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <Package className="w-4 h-4 text-yellow-400" />
                        <span className="font-semibold text-sm text-yellow-400">{alert.package}</span>
                      ...
                      {alert.securityVulnerabilities > 0 && (
                        <div className="text-xs text-red-400 mb-1">
                          🚨 {alert.securityVulnerabilities} security vulnerabilities
                        </div>
                      )}
                      <p className="text-sm text-yellow-300/90 mb-2">{alert.recommendation}</p>
                    </div>
