...
export class ContextualAssistant {
  ...
  async learnCodingStyle(
    ...
    const updatedStyle = this.mergeCodingStyles(
      context.codeStyle,
      analyzedStyle
    )
  ...
  private async analyzeCodingStyle(
    codeFiles: Record<string, string>
  ): Promise<Partial<CodingStyle>> {
...
Return as JSON object with these properties.`
    ...
    try {
      const result = await generateText({
        ...
        system:
          'You are a code style analyzer. Extract consistent patterns from code samples.',
      })
  ...
  private mergeCodingStyles(
    existing: CodingStyle,
    analyzed: Partial<CodingStyle>
  ): CodingStyle {
    return {
      ...existing,
      ...analyzed,
      patterns: [
        ...new Set([...existing.patterns, ...(analyzed.patterns || [])]),
      ],
    }
  }
  ...
  async generateProactiveSuggestions(
    ...
    const featureSuggestions = await this.generateFeatureSuggestions(
      context,
      currentCode
    )
    ...
    return suggestions.sort(
      (a, b) => this.priorityScore(b.priority) - this.priorityScore(a.priority)
    )
  }
...
Consider user needs, business goals, and technical feasibility.`
    ...
    try {
      const result = await generateText({
        ...
        system:
          'You are a product manager and UX expert. Suggest valuable features for software projects.',
      })

      // Parse suggestions from AI response
      const parsedSuggestions = this.parseFeatureSuggestions(
        result.text,
        context
      )
...
Suggest specific performance optimizations with code examples.`
    ...
    try {
      ...
      const parsedSuggestions = this.parsePerformanceSuggestions(
        result.text,
        context
      )
      suggestions.push(...parsedSuggestions)
  ...
  private async generateUXSuggestions(
    context: ProjectContext
  ): Promise<ProactiveSuggestion[]> {
    const suggestions: ProactiveSuggestion[] = []
...
Focus on user experience, accessibility, and usability improvements.`
    ...
    try {
      const result = await generateText({
        ...
        prompt: uxPrompt,
        system:
          'You are a UX/UI expert. Suggest improvements that enhance user experience and accessibility.',
      })
...
Focus on scalable, maintainable architecture.`
    ...
    try {
      const result = await generateText({
        ...
        system:
          'You are a senior software architect. Design scalable database schemas and API architectures.',
      })
  ...
  private parseFeatureSuggestions(
    text: string,
    context: ProjectContext
  ): ProactiveSuggestion[] {
  ...
  private parsePerformanceSuggestions(
    text: string,
    context: ProjectContext
  ): ProactiveSuggestion[] {
    return []
  ...
  private parseBusinessLogicSuggestions(
    text: string
