...
export class PipelineOrchestrator {
  ...
  private async searchSimilarDocuments(
    ...
    // Use external knowledge base for better maintainability
    const knowledgeBase = KNOWLEDGE_BASE

    if (queryEmbedding.length === 0) {
      // Fallback to keyword matching if embedding failed
      return this.fallbackKeywordSearch(userQuery).map((doc) => ({
        ...doc,
        score: 0.5,
      }))
    }
    ...
    return docs
      ...
      .map((doc) => {
        ...
        if (
          ...
          doc.text.toLowerCase().includes('react')
        ) {
          adjustedScore += 0.2
        }
        if (
          queryLower.includes('security') &&
          doc.text.toLowerCase().includes('security')
        ) {
          adjustedScore += 0.2
        }
        if (
          queryLower.includes('performance') &&
          doc.text.toLowerCase().includes('performance')
        ) {
          adjustedScore += 0.2
        }
...
- Regularly refactor and optimize your code for 2025 standards`,
  ...
  private generateSearchQueries(
    ...
    const extractKeyTerms = (text: string): string[] => {
      ...
      return words
        ...
        .slice(0, 2) // Only top 2 key terms
    }
    ...
    return queries
      .map((query) =>
        query.length > 400 ? query.substring(0, 397) + '...' : query
      )
      .slice(0, 3)
  }
  ...
  private detectProjectType(prompt: string): string {
    ...
    if (promptLower.includes('gin') || promptLower.includes('fiber'))
      return 'go'
    if (promptLower.includes('actix') || promptLower.includes('axum'))
      return 'rust'
    ...
    if (promptLower.includes('c#') || promptLower.includes('csharp'))
      return 'csharp'
  ...
  private buildStyleInstructions(stylePreferences?: StylePreferences): string {
    ...
    if (stylePreferences.colorScheme) {
      if (stylePreferences.colorScheme === 'dark') {
        instructions.push(
          'Use a dark theme with dark backgrounds and light text'
        )
      } else if (stylePreferences.colorScheme === 'light') {
        instructions.push(
          'Use a light theme with light backgrounds and dark text'
        )
      }
    }
    ...
    if (stylePreferences.primaryColor) {
      instructions.push(
        `Use ${stylePreferences.primaryColor} as the primary color`
      )
    }

    if (stylePreferences.secondaryColor) {
      instructions.push(
        `Use ${stylePreferences.secondaryColor} as the secondary/accent color`
      )
    }
    ...
    if (stylePreferences.layoutStyle) {
      switch (stylePreferences.layoutStyle) {
        case 'minimal':
          instructions.push(
            'Use a minimal, clean design with lots of whitespace'
          )
          break
        case 'modern':
          instructions.push(
            'Use a modern design with gradients, shadows, and contemporary styling'
          )
          break
        case 'bold':
          instructions.push(
            'Use a bold design with strong contrasts and eye-catching elements'
          )
          break
        case 'classic':
          instructions.push(
            'Use a classic, traditional design with timeless elements'
          )
          break
      }
    }
    ...
    if (stylePreferences.typography) {
      switch (stylePreferences.typography) {
        ...
        case 'monospace':
          instructions.push(
            'Use monospace fonts for a technical, code-like appearance'
          )
          break
        default:
  ...
  async executeFullPipeline(request: PipelineRequest): Promise<PipelineResult> {
    ...
    try {
      for (let iteration = 1; iteration <= this.MAX_RETRIES; iteration++) {
        ...
        const rawCode = await this.generateCode(
          request,
          iteration > 1 ? currentCode : undefined
        )
        ...
        await pipelineLogger.logStage(
          request.requestId,
          `ITERATION_${iteration}_COMPLETE`,
          pipelineIteration,
          null,
          0
        )
        ...
        if (
          ...
          request.prompt = this.enhancePromptWithFeedback(
            request.prompt,
            review
          )
          continue
      ...
      if (request.mode === 'codegen' && currentCode) {
        ...
        if (parsedFiles) {
          ...
          const baseStructure = this.generateProjectStructure(
            projectType,
            request.prompt
          )
...
`,
  ...
  private getSetupInstructions(projectType: string): string {
    ...
    return (
      instructions[projectType] ||
      'Follow the standard setup process for this project type.'
    )
  }
...
Generate improved code that addresses the feedback.`
      ...
      const realTimeKnowledge = await this.searchRealTimeKnowledge(
        enhancedPrompt,
        previousCode
      )
...
Generate ONLY the code, no explanations:`
  ...
  private async reviewCode(
    ...
    try {
      const projectType =
        rawCode.metadata?.projectType || this.detectProjectType(rawCode.prompt)
...
Be thorough but constructive in your review.`
      ...
      return {
        ...
        qualityScore: 5,
        securityIssues: [
          'Review system unavailable - manual review recommended',
        ],
        performanceIssues: [],
  ...
  private parseReviewResponse(reviewText: string): {
    ...
    try {
      ...
      const securityMatch = reviewText.match(
        /SECURITY_ISSUES:\s*(.+?)(?=\n|$)/s
      )
      const performanceMatch = reviewText.match(
        /PERFORMANCE_ISSUES:\s*(.+?)(?=\n|$)/s
      )
      const verdictMatch = reviewText.match(
        /VERDICT:\s*(APPROVE|REJECT|NEEDS_REVISION)/
      )
      ...
      return {
        ...
        securityIssues:
          securityMatch && securityMatch[1] !== 'NONE'
            ? [securityMatch[1].trim()]
            : [],
        performanceIssues:
          performanceMatch && performanceMatch[1] !== 'NONE'
            ? [performanceMatch[1].trim()]
            : [],
        verdict: (verdictMatch?.[1] as any) || 'NEEDS_REVISION',
        suggestedFixes:
          fixesMatch && fixesMatch[1] !== 'NONE' ? [fixesMatch[1].trim()] : [],
      }
    } catch (error) {
      // Fallback parsing
      return {
  ...
  private enhancePromptWithFeedback(
