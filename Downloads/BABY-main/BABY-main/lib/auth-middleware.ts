...
export async function requireAuth(req: NextRequest) {
  ...
  if (apiKey) {
    ...
    if (!rateLimitResult.success) {
      throw new RateLimitError(
        `API rate limit exceeded. Try again in ${rateLimitResult.retryAfter} seconds`
      )
    }
  ...
  if (!sessionId) {
    throw new AuthenticationError(
      'Authentication required - provide session token or API key'
    )
  }
...
export async function checkOTPRateLimit(req: NextRequest, email: string) {
  ...
  if (!result.success) {
    throw new RateLimitError(
      `Too many OTP requests. Try again in ${result.retryAfter} seconds`
    )
  }
...
export async function checkChatRateLimit(req: NextRequest, userId: string) {
  ...
  if (!result.success) {
    throw new RateLimitError(
      `Chat rate limit exceeded. Try again in ${result.retryAfter} seconds`
    )
  }
