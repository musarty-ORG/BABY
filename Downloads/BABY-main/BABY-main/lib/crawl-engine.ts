...
export class CrawlEngine {
  ...
  private generateSummary(results: CrawlResult[]) {
    ...
    return {
      ...
      avgContentLength:
        results.length > 0
          ? Math.round(totalContentLength / results.length)
          : 0,
      totalImages,
      totalLinks,
      errorPages,
  ...
  async crawlDocumentation(
    ...
    return this.crawl(url, {
      ...
      instructions:
        'Extract comprehensive documentation content including code examples, API references, and guides',
      ...options,
    })
  }
  ...
  async crawlBlog(
    ...
    return this.crawl(url, {
      ...
      instructions:
        'Extract blog posts and articles with publication dates and author information',
      ...options,
    })
  }
  ...
  async crawlAPI(
    ...
    return this.crawl(url, {
      ...
      instructions:
        'Extract API documentation including endpoints, parameters, examples, and response formats',
      ...options,
    })
  }

  async crawlChangelog(
    url: string,
    options: Partial<CrawlOptions> = {}
  ): Promise<CrawlResponse> {
    return this.crawl(url, {
      ...
      instructions:
        'Extract version history, release notes, and change information with dates',
      ...options,
    })
  }

  async crawlGitHubRepo(
    repoUrl: string,
    options: Partial<CrawlOptions> = {}
  ): Promise<CrawlResponse> {
    return this.crawl(repoUrl, {
      ...
      instructions:
        'Extract README files, documentation, code examples, and project structure',
      includeRawHtml: true,
      ...options,
    })
  }
  ...
  private generateCombinedInsights(
    ...
    const crawlUrls = new Set(
      crawlResults.flatMap((cr) => cr.results.map((r) => r.url))
    )

    const uniqueToCrawl = Array.from(crawlUrls).filter(
      (url) => !searchUrls.has(url)
    )
    if (uniqueToCrawl.length > 0) {
      insights.push(
        `Crawling discovered ${uniqueToCrawl.length} additional relevant pages not found in search`
      )
    }
    ...
    const avgCrawlContentLength =
      crawlResults.reduce(
        (sum, cr) => sum + (cr.summary?.avgContentLength || 0),
        0
      ) / crawlResults.length
    ...
    const categories = new Set(
      crawlResults.flatMap((cr) => Object.keys(cr.summary?.categories || {}))
    )
  ...
  private async getCachedResult(
    cacheKey: string
  ): Promise<Omit<CrawlResponse, 'crawlTime' | 'cached'> | null> {
    try {
  ...
  private async cacheResult(
    cacheKey: string,
    result: Omit<CrawlResponse, 'crawlTime' | 'cached'>
  ): Promise<void> {
    try {
  ...
  async clearCache(pattern?: string): Promise<number> {
    try {
      const keys = pattern
        ? await redis.keys(`crawl:*${pattern}*`)
        : await redis.keys('crawl:*')
