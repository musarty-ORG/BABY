...
export class PayPalService {
  ...
  private async getAccessToken(): Promise<string> {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `PayPal auth failed: ${response.statusText} - ${errorText}`
      )
    }
  ...
  private getPlanId(plan: string): string {
    ...
    if (!planId) {
      throw new Error(
        `Invalid plan: ${plan}. Available plans: basic, builder, architect`
      )
    }
  ...
  async createSubscription(
    ...
    console.log(
      `Creating subscription for plan: ${planId} -> PayPal Plan ID: ${actualPlanId}`
    )
  ...
  async getSubscription(subscriptionId: string): Promise<PayPalSubscription> {
    ...
    const response = await fetch(
      `${this.config.baseUrl}/v1/billing/subscriptions/${subscriptionId}`,
      {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${accessToken}`,
  ...
  async cancelSubscription(
    ...
    const response = await fetch(
      `${this.config.baseUrl}/v1/billing/subscriptions/${subscriptionId}/cancel`,
      {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${accessToken}`,
  ...
  async createCustomer(
    ...
    const response = await fetch(
      `${this.config.baseUrl}/v2/customer/customers`,
      {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${accessToken}`,
  ...
  async getVaultedPaymentMethods(
    customerId: string
  ): Promise<VaultedPaymentMethod[]> {
    const accessToken = await this.getAccessToken()

    const response = await fetch(
      `${this.config.baseUrl}/v2/customer/customers/${customerId}/payment-methods`,
      {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${accessToken}`,
  ...
  async deleteVaultedPaymentMethod(vaultId: string): Promise<void> {
    ...
    const response = await fetch(
      `${this.config.baseUrl}/v2/customer/payment-methods/${vaultId}`,
      {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${accessToken}`,
