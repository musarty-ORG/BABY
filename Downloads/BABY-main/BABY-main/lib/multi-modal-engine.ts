...
export class MultiModalEngine {
  ...
  private async enhanceVoiceCommand(
...
Provide detailed analysis for app development.`
      ...
      const result = await this.safeApiCall(async () => {
        return await generateText({
          ...
          prompt: sketchPrompt,
          system:
            'You are an expert UX analyst. Convert sketches and wireframes to technical specifications.',
        })
      })
  ...
  private parseSketchAnalysis(analysisText: string): SketchAnalysis {
    try {
      ...
      return {
        ...
        technicalRequirements:
          this.extractTechRequirementsFromText(analysisText),
      }
    } catch (error) {
...
Focus on extracting actionable development requirements.`
      ...
      const result = await this.safeApiCall(async () => {
        return await generateText({
          ...
          prompt: videoPrompt,
          system:
            'You are a video analysis expert for software development. Extract requirements from video content.',
        })
      })
