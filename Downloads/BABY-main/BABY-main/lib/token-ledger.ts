...
export class TokenLedgerService {
  ...
  constructor() {
    this.tokensPerMessage = Number.parseInt(
      process.env.TOKENS_PER_MSG || '1000'
    )
    this.billingCurrency = process.env.BILLING_CURRENCY || 'USD'
  }

  async addEntry(
    entry: Omit<TokenLedgerEntry, 'id' | 'created_at'>
  ...
  async billUser(
    ...
    if (balance.total_balance < messages) {
      console.log(
        `Insufficient balance for user ${userId}: need ${messages}, have ${balance.total_balance}`
      )
  ...
  async resetMonthlyBucket(
    userId: string,
    planMessages: number
  ): Promise<void> {
    console.log(
      `Resetting monthly bucket for user ${userId}: ${planMessages} messages`
    )
