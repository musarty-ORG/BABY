...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!userId || !projectData) {
      return Response.json(
        { error: 'userId and projectData are required' },
        { status: 400 }
      )
    }

    const context = await contextualAssistant.createProjectContext(
      userId,
      projectData
    )
...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (!userId || !projectId || !updates) {
      return Response.json(
        { error: 'userId, projectId, and updates are required' },
        { status: 400 }
      )
    }

    const context = await contextualAssistant.updateProjectContext(
      userId,
      projectId,
      updates
    )
