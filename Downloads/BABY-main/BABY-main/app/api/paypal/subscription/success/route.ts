  ...
  try {
    ...
    if (!subscriptionId) {
      return NextResponse.redirect(
        `${process.env.NEXT_PUBLIC_BASE_URL}/pricing?error=missing_subscription_id`
      )
    }
    ...
    if (!dbSubscription) {
      ...
      return NextResponse.redirect(
        `${process.env.NEXT_PUBLIC_BASE_URL}/pricing?error=subscription_not_found`
      )
    }
    ...
    if (planName) {
      ...
      const messages =
        messageAllocation[planName as keyof typeof messageAllocation]
      if (messages) {
        await tokenLedger.creditUser(
          dbSubscription.user_id,
          messages,
          `Initial ${planName} plan allocation`,
          'monthly'
        )
      }
    }
    ...
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_BASE_URL}/pricing?success=subscription_activated`
    )
  } catch (error: any) {
