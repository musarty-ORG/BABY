...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!targetUrls || !Array.isArray(targetUrls) || targetUrls.length === 0) {
      return Response.json(
        { error: 'targetUrls is required and must be a non-empty array' },
        { status: 400 }
      )
    }
    ...
    if (targetUrls.length > 5) {
      return Response.json(
        { error: 'Maximum 5 target URLs allowed' },
        { status: 400 }
      )
    }
    ...
    if (!process.env.TAVILY_API_KEY) {
      return Response.json(
        { error: 'Search and crawl service not configured' },
        { status: 500 }
      )
    }

    const result = await crawlEngine.searchAndCrawl(
      query,
      targetUrls,
      crawlOptions
    )
