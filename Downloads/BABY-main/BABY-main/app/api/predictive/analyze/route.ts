...
export const POST = withErrorHandler(async (req: NextRequest) => {
  ...
  if (!codeFiles || !projectType) {
    return Response.json(
      { error: 'Missing required fields: codeFiles and projectType' },
      { status: 400 }
    )
  }
  ...
  for (const [path, code] of Object.entries(codeFiles)) {
    if (typeof code === 'string') {
      // Detect code smells
      const smells = await predictiveEngine.detectCodeSmells(
        code,
        path,
        projectType
      )
      results.codeSmells.push(
        ...smells.map((smell) => ({ ...smell, file: path }))
      )

      // Generate refactoring opportunities
      const opportunities =
        await predictiveEngine.generateRefactoringOpportunities(
          code,
          path,
          projectType
        )
      results.refactoringOpportunities.push(
        ...opportunities.map((opp) => ({ ...opp, file: path }))
      )
  ...
  if (codeFiles['package.json']) {
    try {
      const dependencyAlerts = await predictiveEngine.checkDependencyHealth(
        codeFiles['package.json']
      )
  ...
  const insights = await predictiveEngine.generatePredictiveInsights(
    session.id,
    projectType,
    codeFiles
  )
  ...
  return Response.json({
    ...
    summary: {
      ...
      criticalIssues: results.codeSmells.filter(
