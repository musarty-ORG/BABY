...
async function handleDualModelGeneration(
  ...
  try {
    await pipelineLogger.logStage(
      ...
      'DUAL_MODEL_START',
      null,
      'Starting dual model generation',
      0
    )
...
Generate clean, functional code ready for deployment.`
    ...
    const model1Promise = streamText({
      ...
      prompt: basePrompt,
      system:
        'You are Claude, an expert web developer. Create complete, production-ready code with modern patterns and best practices.',
    })
    ...
    const model2Promise = streamText({
      ...
      prompt: basePrompt,
      system:
        'You are Llama, a powerful code generation AI. Focus on clean, efficient, and modern code architecture.',
    })
    ...
    if (model1Result.status === 'fulfilled') {
      await pipelineLogger.logStage(
        ...
        'DUAL_MODEL_COMPLETE',
        null,
        'Primary model (Claude) succeeded',
        0
      )
      ...
      await pipelineLogger.logStage(
        ...
        'DUAL_MODEL_COMPLETE',
        null,
        'Secondary model (Llama) succeeded',
        0
      )
    ...
    await pipelineLogger.logError(
      request.requestId,
      'DUAL_MODEL',
      `Dual model generation failed: ${error}`,
      false
    )
    throw error
  }
...
Generate the code without explanations, just the code:`
    ...
    await pipelineLogger.logError(
      request.requestId,
      'CODE_GEN',
      `Streaming code generation failed: ${error}`,
      false
    )
    throw error
  }
...
Be thorough but constructive in your review.`
    ...
    await pipelineLogger.logError(
      request.requestId,
      'REVIEW',
      `Streaming code review failed: ${error}`,
      false
    )
    throw error
  }
...
async function handleFullPipeline(request: PipelineRequest, session: any) {
  try {
    ...
    await pipelineLogger.logError(
      request.requestId,
      'ORCHESTRATION',
      `Full pipeline execution failed: ${error}`,
      false
    )
