...
export default function MultiAgentPipeline() {
  ...
  const [useDualModel, setUseDualModel] = useState(true) // Default to dual model for power
    ...
    try {
      ...
      const response = await fetch('/api/multi-agent', {
        ...
        body: JSON.stringify({
          ...
          agentMode: useDualModel ? 'dual-model' : 'full-pipeline',
          mode: 'codegen',
          stylePreferences,
        }),
      })
...
`
  ...
  const getPreviewHTML = () => {
    if (!pipelineResult?.codeFiles)
      return '<html><body><h1>No preview available</h1></body></html>'

    // Try to find the main HTML file or create a basic preview
    const mainFile =
      pipelineResult.codeFiles['index.html'] ||
      pipelineResult.codeFiles['app/page.tsx'] ||
      pipelineResult.codeFiles['pages/index.tsx'] ||
      Object.values(pipelineResult.codeFiles)[0]
    ...
    if (
      mainFile.includes('export default') ||
      mainFile.includes('function') ||
      mainFile.includes('const')
    ) {
      return `<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    ...
    <script type="text/babel">
        try {
          ${mainFile.replace(/import.*from.*;/g, '').replace(/export default /g, 'const Component = ')}
          
          const element = React.createElement(Component || (() => React.createElement('div', {}, 'Component loaded')));
          ReactDOM.render(element, document.getElementById('root'));
        } catch (error) {
          document.getElementById('root').innerHTML = '<div style="padding: 20px; background: #fee; color: #c33; border-radius: 8px;"><h3>Preview Error</h3><p>Could not render component preview. This is normal for complex components that require build tools.</p><p><strong>Error:</strong> ' + error.message + '</p></div>';
        }
    </script>
</body>
</html>`
    ...
    if (mainFile.includes('<html') || mainFile.includes('<!DOCTYPE')) {
      return mainFile
    }
...
<body>
    <div class="preview-content">
        ...
        <p>Your website has been generated with ${Object.keys(pipelineResult.codeFiles).length} files.</p>
        <p>Click "Deploy to Vercel" to see the full interactive version, or download the ZIP to run locally.</p>
        ...
        ${
          adminSettings.enableRegistries
            ? `
        <h3>📦 Available Component Registries:</h3>
        <ul>
            <li><strong>shadcn/ui:</strong> Modern React components</li>
            <li><strong>Headless UI:</strong> Unstyled, accessible components</li>
            <li><strong>Radix UI:</strong> Low-level UI primitives</li>
            <li><strong>Chakra UI:</strong> Simple, modular components</li>
        </ul>
        `
            : ''
        }
  ...
  return (
    <div className="min-h-screen bg-black text-white font-mono overflow-hidden">
      ...
      <SiteHeader
        ...
        subtitle="AI-POWERED. REAL-TIME. PRODUCTION-READY."
        variant="builder"
        showStatus={false}
      />
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gray-800/30 border border-cyan-500/20 rounded-lg p-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-2 text-xs">
                    ...
                    <span className="text-cyan-400">
                      {useDualModel ? 'DUAL MODEL (5X FASTER)' : 'SINGLE MODEL'}
                    </span>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  <button
                    type="button"
                    onClick={() => setUseDualModel(false)}
                    className={`flex-1 px-4 py-2 rounded-lg transition-all duration-200 text-sm font-semibold ${
                      !useDualModel
                  ...
                  <button
                    type="button"
                    onClick={() => setUseDualModel(true)}
                    className={`flex-1 px-4 py-2 rounded-lg transition-all duration-200 text-sm font-semibold ${
                      useDualModel
                        ? 'bg-gradient-to-r from-cyan-600/30 to-purple-600/30 text-cyan-300 border border-cyan-500/50'
                        : 'bg-gray-700/50 text-gray-400 border border-gray-600/50 hover:bg-gray-700'
                    }`}
                    disabled={isProcessing}
                  >
                ...
                <p className="text-xs text-cyan-500/70 mt-2">
                  {useDualModel
                    ? 'Claude + Llama working in parallel for maximum power and speed'
                    : 'Single AI model generation (slower but uses fewer tokens)'}
                </p>
              </div>
              ...
              <div className="bg-gray-800/30 border border-cyan-500/20 rounded-lg p-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  >
                    ...
                    <span>
                      {showRegistries ? 'HIDE REGISTRIES' : 'SHOW AVAILABLE'}
                    </span>
                  </button>
                </div>
                ...
                {showRegistries && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
                    {AVAILABLE_REGISTRIES.slice(0, 4).map((registry) => (
                      <div
                        key={registry.name}
                        className="bg-gray-700/30 border border-cyan-500/20 rounded p-3"
                      >
                ...
                <p className="text-xs text-cyan-500/70 mt-2">
                  AI will automatically suggest and use appropriate components
              ...
              <div>
                ...
                >
                  ...
                  <span
                    className={`transform transition-transform ${showStyleOptions ? 'rotate-180' : ''}`}
                  >
                    ▼
                  </span>
                </button>
            ...
            <div className="bg-gray-900/50 border border-purple-500/30 rounded-lg p-4 backdrop-blur-sm">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-4 text-xs">
                  ...
                  <span
                    className={`font-semibold ${getStatusColor(pipelineResult.status)}`}
                  >
                    {pipelineResult.status}
                  </span>
                  <span className="text-purple-500/70">
                    {pipelineResult.totalTime}ms
                  </span>
                  <button
              ...
              {/* Error Log */}
              {pipelineResult.errorLog &&
                pipelineResult.errorLog.length > 0 && (
                  <div className="mb-4 bg-red-900/20 border border-red-500/30 rounded-lg p-3">
                    ...
                    {pipelineResult.errorLog.map((error, index) => (
                      <div key={index} className="text-xs text-red-300 mb-1">
                        <span className="text-red-500">[{error.stage}]</span>{' '}
                        {error.error}
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    ...
                    <div className="bg-gray-800/50 border border-purple-500/20 rounded-lg p-4">
                      ...
                      <div className="space-y-2">
                        {Object.keys(pipelineResult.codeFiles).map(
                          (filePath) => (
                            <button
                              key={filePath}
          ...
          <div className="bg-gray-900/95 border border-red-500/30 rounded-lg p-6 max-w-2xl w-full max-h-[80vh] overflow-y-auto">
            ...
            <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setAdminSettings({
                          ...adminSettings,
                          secondaryModel: e.target.value,
                        })
                      }
                      className="w-full bg-gray-700/50 border border-red-500/30 rounded px-3 py-2 text-red-100 text-sm focus:outline-none focus:border-red-400"
                    >
                      <option value="meta-llama/llama-3.1-70b-versatile">
                        Llama 3.1 70B
                      </option>
                      <option value="meta-llama/llama-3.1-8b-instant">
                        Llama 3.1 8B
                      </option>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setAdminSettings({
                          ...adminSettings,
                          enableRealTimeSearch: e.target.checked,
                        })
                      }
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setAdminSettings({
                          ...adminSettings,
                          enableRegistries: e.target.checked,
                        })
                      }
                      className="w-4 h-4 text-red-400 bg-gray-700 border-red-500 rounded focus:ring-red-400"
                    />
              ...
              <div className="text-xs text-red-500/70 text-center">
                Access: Ctrl+Shift+A+D+M | Settings saved locally | Use
                responsibly
              </div>
            </div>
          </div>
