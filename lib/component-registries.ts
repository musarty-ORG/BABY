...
export const AVAILABLE_REGISTRIES: ComponentRegistry[] = [
  ...
  },
  {
    ...
    components: [
      ...
      {
        name: "Alert",
        description: "Component to communicate state and contextual information",
...
export function searchComponents(query: string): ComponentInfo[] {
  const allComponents = AVAILABLE_REGISTRIES.flatMap(registry => registry.components)
...
export function getComponentsByCategory(category: string): ComponentInfo[] {
  const allComponents = AVAILABLE_REGISTRIES.flatMap(registry => registry.components)
  return allComponents.filter(component => 
    component.tags.some(tag => tag.toLowerCase().includes(category.toLowerCase()))
  )
}
...
Popular Components:
${registry.components.slice(0, 3).map(comp => `- ${comp.name}: ${comp.description}`).join('\n')}
